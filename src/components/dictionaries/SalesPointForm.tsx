...
import { SalesPoint, UpdateSalesPointRequest } from 'schema/serverTypes';
...
export type SalesPointFormProps = {
  onSuccess: () => void;
  item?: SalesPoint;
};

const useSalesPointForm = (message: string, onSuccess: () => void, item?: SalesPoint) => {
  const { handleSubmit, control, formState, setError, clearErrors, reset } =
    useForm<UpdateSalesPointRequest>({
      mode: 'onBlur',
      ...
      queryClient.invalidateQueries({
        predicate: (query) => {
          return (
            ...
            (query.queryKey as string).startsWith('salesPoints')
          );
        },
      });
    },
  ...
  const onSubmit = useMemo(() => {
    const submit = async (form: UpdateSalesPointRequest) => {
      try {
...
export const SalesPointForm = (props: SalesPointFormProps) => {
  const classes = useStyles();
  const { onSuccess, item } = props;
  const { t } = useTranslation();
  const message = t('Could not create');
  const { onSubmit, isSubmitting, control } = useSalesPointForm(message, onSuccess, item);
  ...
  return (
    <form>
      ...
      <Grid container columnSpacing={2} rowSpacing={2.5}>
        ...
        <Grid item md={24} xs={24}>
          <Typography className={classes.required}>*обязательные поля</Typography>
        </Grid>
        <Grid item md={24} xs={24}>
          <Button
